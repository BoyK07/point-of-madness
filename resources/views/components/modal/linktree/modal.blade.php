<div>
    <!-- Global Modals -->
    @php
        $linkInstagram = ssot_links('social.instagram')->first();
        $linkTiktok = ssot_links('social.tiktok')->first();
        $linkSpotify = ssot_links('streaming.spotify')->first();
        $linkAmazon = ssot_links('streaming.amazon_music')->first();
        $linkDeezer = ssot_links('streaming.deezer')->first();
        $linkTidal = ssot_links('streaming.tidal')->first();
        $linkQobuz = ssot_links('streaming.qobuz')->first();
        $linkYoutube = ssot_links('social.youtube')->first();
        $linkYoutubeMusic = ssot_links('streaming.youtube_music')->first();
        $linkVideoToxic = ssot_links('video.toxic')->first();
    @endphp
    ...
    <x-modal name="linktree" maxWidth="lg">
        <div class="bg-gradient-to-br from-gray-900/90 to-black/90 backdrop-blur-sm p-8 text-white border border-white/10">
            ...
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h2 class="text-3xl font-bold text-white">@phrase('modal.linktree.title', 'Point of Madness')</h2>
                    <p class="text-gray-400 text-sm mt-1">@phrase('modal.linktree.subtitle', 'All Links & Platforms')</p>
                </div>
            ...
            <div class="mb-8">
                <h3 class="text-lg font-semibold text-gray-300 mb-4 flex items-center gap-2">
                    ...
                    @phrase('modal.linktree.section.social', 'Social Media')
                </h3>
                <div class="grid gap-3">
                    <a href="{{ $linkInstagram?->url ?? '#' }}" target="{{ $linkInstagram?->target ?? '_blank' }}"
                       @if($linkInstagram?->rel) rel="{{ $linkInstagram->rel }}" @endif
                       class="flex items-center gap-4 p-4 bg-gradient-to-r from-pink-600 to-purple-600
                        ...
                        <div>
                            <div class="font-semibold">{{ $linkInstagram?->label ?? phrase('modal.linktree.instagram.label', 'Instagram') }}</div>
                            <div class="text-sm opacity-80">@phrase('modal.linktree.instagram.handle', '@pointofmadnessband')</div>
                        </div>
                    </a>

                    <a href="{{ $linkTiktok?->url ?? '#' }}" target="{{ $linkTiktok?->target ?? '_blank' }}"
                       @if($linkTiktok?->rel) rel="{{ $linkTiktok->rel }}" @endif
                       class="flex items-center gap-4 p-4 bg-gradient-to-r from-black to-gray-800
                        ...
                        <div>
                            <div class="font-semibold">{{ $linkTiktok?->label ?? phrase('modal.linktree.tiktok.label', 'TikTok') }}</div>
                            <div class="text-sm opacity-80">@phrase('modal.linktree.tiktok.handle', '@point.of.madness')</div>
                        </div>
                    </a>
                </div>
            </div>
            ...
            <div class="mb-8">
                <h3 class="text-lg font-semibold text-gray-300 mb-4 flex items-center gap-2">
                    ...
                    @phrase('modal.linktree.section.streaming', 'Music Streaming')
                </h3>
                <div class="grid gap-3">
                    <a href="{{ $linkSpotify?->url ?? '#' }}" target="{{ $linkSpotify?->target ?? '_blank' }}"
                       @if($linkSpotify?->rel) rel="{{ $linkSpotify->rel }}" @endif
                       class="flex items-center gap-4 p-4 bg-gradient-to-r from-green-600 to-green-500
                        ...
                        <div>
                            <div class="font-semibold">{{ $linkSpotify?->label ?? phrase('modal.linktree.spotify.label', 'Spotify') }}</div>
                            <div class="text-sm opacity-80">@phrase('modal.linktree.spotify.description', '955 monthly listeners')</div>
                        </div>
                    </a>

                    <a href="{{ $linkAmazon?->url ?? '#' }}" target="{{ $linkAmazon?->target ?? '_blank' }}"
                       @if($linkAmazon?->rel) rel="{{ $linkAmazon->rel }}" @endif
                       class="flex items-center gap-4 p-4 bg-gradient-to-r from-orange-600 to-yellow-500
                        ...
                        <div>
                            <div class="font-semibold">{{ $linkAmazon?->label ?? phrase('modal.linktree.amazon.label', 'Amazon Music') }}</div>
                            <div class="text-sm opacity-80">@phrase('modal.linktree.amazon.description', 'Stream on Amazon')</div>
                        </div>
                    </a>

                    <a href="{{ $linkDeezer?->url ?? '#' }}" target="{{ $linkDeezer?->target ?? '_blank' }}"
                       @if($linkDeezer?->rel) rel="{{ $linkDeezer->rel }}" @endif
                       class="flex items-center gap-4 p-4 bg-gradient-to-r from-purple-600 to-pink-500
                        ...
                        <div>
                            <div class="font-semibold">{{ $linkDeezer?->label ?? phrase('modal.linktree.deezer.label', 'Deezer') }}</div>
                            <div class="text-sm opacity-80">@phrase('modal.linktree.deezer.description', 'High quality streaming')</div>
                        </div>
                    </a>

                    <a href="{{ $linkTidal?->url ?? '#' }}" target="{{ $linkTidal?->target ?? '_blank' }}"
                       @if($linkTidal?->rel) rel="{{ $linkTidal->rel }}" @endif
                       class="flex items-center gap-4 p-4 bg-gradient-to-r from-blue-900 to-cyan-700
                        ...
                        <div>
                            <div class="font-semibold">{{ $linkTidal?->label ?? phrase('modal.linktree.tidal.label', 'Tidal') }}</div>
                            <div class="text-sm opacity-80">@phrase('modal.linktree.tidal.description', 'Hi-Fi music streaming')</div>
                        </div>
                    </a>

                    <a href="{{ $linkQobuz?->url ?? '#' }}" target="{{ $linkQobuz?->target ?? '_blank' }}"
                       @if($linkQobuz?->rel) rel="{{ $linkQobuz->rel }}" @endif
                       class="flex items-center gap-4 p-4 bg-gradient-to-r from-indigo-700 to-purple-700
                        ...
                        <div>
                            <div class="font-semibold">{{ $linkQobuz?->label ?? phrase('modal.linktree.qobuz.label', 'Qobuz') }}</div>
                            <div class="text-sm opacity-80">@phrase('modal.linktree.qobuz.description', 'Studio quality')</div>
                        </div>
                    </a>

                    <a href="{{ $linkYoutube?->url ?? '#' }}" target="{{ $linkYoutube?->target ?? '_blank' }}"
                       @if($linkYoutube?->rel) rel="{{ $linkYoutube->rel }}" @endif
                       class="flex items-center gap-4 p-4 bg-gradient-to-r from-red-600 to-red-500
                        ...
                        <div>
                            <div class="font-semibold">{{ $linkYoutube?->label ?? phrase('modal.linktree.youtube.label', 'YouTube') }}</div>
                            <div class="text-sm opacity-80">@phrase('modal.linktree.youtube.description', 'Music & videos')</div>
                        </div>
                    </a>

                    <a href="{{ $linkYoutubeMusic?->url ?? '#' }}" target="{{ $linkYoutubeMusic?->target ?? '_blank' }}"
                       @if($linkYoutubeMusic?->rel) rel="{{ $linkYoutubeMusic->rel }}" @endif
                       class="flex items-center gap-4 p-4 bg-gradient-to-r from-red-800 to-orange-600
                        ...
                        <div>
                            <div class="font-semibold">{{ $linkYoutubeMusic?->label ?? phrase('modal.linktree.youtube_music.label', 'YouTube Music') }}</div>
                            <div class="text-sm opacity-80">@phrase('modal.linktree.youtube_music.description', 'Dedicated music streaming')</div>
                        </div>
                    </a>
                </div>
            </div>
            ...
            <div class="mb-8">
                <h3 class="text-lg font-semibold text-gray-300 mb-4 flex items-center gap-2">
                    ...
                    @phrase('modal.linktree.section.videos', 'Music Videos')
                </h3>
                <div class="grid gap-3">

                    <a href="{{ $linkVideoToxic?->url ?? '#' }}" target="{{ $linkVideoToxic?->target ?? '_blank' }}"
                       @if($linkVideoToxic?->rel) rel="{{ $linkVideoToxic->rel }}" @endif
                       class="flex items-center gap-4 p-4 bg-gradient-to-r from-gray-700 to-gray-600
                        ...
                        <div>
                            <div class="font-semibold">@phrase('modal.linktree.video.toxic.title', 'TOXIC - Music Video')</div>
                            <div class="text-sm opacity-80">@phrase('modal.linktree.video.toxic.description', 'Official video clip')</div>
                        </div>
                    </a>
                </div>
            </div>
            ...
            <div class="text-center pt-6 border-t border-gray-700">
                <p class="text-sm text-gray-400 mb-2">@phrase('modal.linktree.footer.tagline', '🎵 Three-piece band • 80s New Wave Revival • Est. 2023')</p>
                <p class="text-xs text-gray-500">@phrase('modal.linktree.footer.latest', 'Latest: "Secondary" (2025) • Follow for updates and tour dates')</p>
            </div>
        </div>
    </x-modal>
</div>