        ...
        <div class="bg-gray-800/70 backdrop-blur-sm border border-gray-600/50 rounded-2xl p-6 shadow-2xl shadow-black/50">
            ...
            <div class="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div class="flex flex-col md:flex-row gap-4 md:items-center md:justify-end">
                    <form method="GET" action="{{ route('admin.events.index') }}" class="flex flex-col sm:flex-row sm:items-center gap-3">
                        <div class="relative w-full sm:w-auto">
                            <input type="text" name="q" value="{{ $search ?? '' }}" placeholder="Zoek op titel of locatie"
                                   class="w-full sm:w-64 md:w-80 rounded-xl bg-gray-900/70 border border-gray-700/60 text-gray-200 placeholder-gray-500 px-10 py-2 focus:outline-none focus:border-orange-500/60 focus:ring-0">
                            <svg class="w-5 h-5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35M10 18a8 8 0 100-16 8 8 0 000 16z" />
                            </svg>
                        </div>
                        @if(!empty($search))
                            <a href="{{ route('admin.events.index') }}" class="px-3 py-2 rounded-lg bg-gray-800/70 border border-gray-700/60 text-gray-300 hover:text-white hover:border-gray-500 transition text-center">Wissen</a>
                        @endif
                        <button type="submit"
                    ...
                    <a href="{{ route('admin.events.create') }}"
                       class="group inline-flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-orange-600 to-red-600
                              text-white font-semibold rounded-xl hover:from-orange-500 hover:to-red-500
                              transition-all duration-300 transform hover:scale-105 hover:shadow-xl hover:shadow-orange-500/30 w-full md:w-auto">
